...
const playlistIdFromUrl = (value?: string) => {
...
async function inspectArchive(file: File): Promise<Inspection> {
  ...
  const metadataChecks: Array<[keyof PackageManifest, string]> = [
    ['title', 'Title'], ['slug', 'Slug'], ['excerpt', 'Excerpt'], ['section', 'Section'], ['publishedAt', 'Publication date'], ['status', 'Status'],
  ];
  metadataChecks.forEach(([field, label]) => {
    const expected = String(manifest[field] ?? '');
    const actual = extractField(indexSource, field);
    add('Metadata', `${label} matches index.ts`, Boolean(indexSource) && actual === expected,
      actual === expected ? expected : `Manifest: ${expected || '—'} | index.ts: ${actual || 'not found'}`);
  });
  ...
  const isPlaylistPost = manifest.section === 'music-playlists' || Boolean(manifest.playlistLinks);
  if (isPlaylistPost) {
    const links = manifest.playlistLinks;
    const youtubeId = playlistIdFromUrl(links?.youtube);
    const musicId = playlistIdFromUrl(links?.youtubeMusic);
    ...
    add('Playlist', 'YouTube playlist URL is valid', validHttpUrl(links?.youtube) && links?.youtube.includes('youtube.com/playlist'), links?.youtube ?? 'Missing YouTube playlist URL.');
    add('Playlist', 'YouTube Music URL is valid', validHttpUrl(links?.youtubeMusic) && links?.youtubeMusic.includes('music.youtube.com/playlist'), links?.youtubeMusic ?? 'Missing YouTube Music URL.');
  ...
  return {
    ...
    manifest,
    files,
    sourceFiles: files.filter((name) => name.startsWith(sourcePrefix)),
    productionFiles: files.filter((name) => name.startsWith(dropInPrefix)),
    images,
    validation,
  };
}
...
function App() {
  ...
  const loadFile = async (file?: File) => {
    ...
    setLoading(true); setError(''); setInspection(null); setSelectedImage(null);
    try {
      const result = await inspectArchive(file);
      setInspection(result); setSelectedImage(result.images[0] ?? null);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'The package could not be inspected.'); }
  ...
  return <main className="app-shell">
    ...
    {inspection && summary && <>
      ...
      <footer className="action-bar"><div><strong>{inspection.archiveName}</strong><span>{summary.ready ? 'The package is ready for the site-preview step.' : 'Resolve failed checks before previewing.'}</span></div><button disabled={!summary.ready}>Build site preview</button></footer>
    </>}
  </main>;
}
